...
public:
    ...
    GewekeTester(size_t num_rows, size_t num_cols,
                 std::vector<std::string> datatypes, unsigned int seed,
                 size_t m=1, bool do_hypers=true, bool do_row_alpha=true,
                 bool do_col_alpha=true, bool do_row_z=true,
                 bool do_col_z=true, size_t ct_kernel=0);
    ...
    std::vector<std::map<std::string, std::vector<double>>> getForwardStats();
    std::vector<std::map<std::string, std::vector<double>>> getPosteriorStats();
    std::vector<double> getStateAlphaForward();
    std::vector<double> getStateAlphaPosterior();
    std::vector<size_t> getNumViewsForward();
    std::vector<size_t> getNumViewsPosterior();

    template <typename T>
    std::vector<std::string> __getMapKeys(std::map<std::string, T> map_in);

    template <typename T>
    std::vector<double> __getDataStats(const std::vector<T> &data,
                                       size_t categorial_k);

    void __updateStats(const baxcat::State &state,
        std::vector<double> &state_crp_alpha,
        std::vector<std::map<std::string, std::vector<double>>> &all_stats,
        std::vector<size_t> &num_views);

    void __initStats(const baxcat::State &state,
        std::vector<double> &state_crp_alpha,
        std::vector<std::map<std::string, std::vector<double>>> &all_stats,
        std::vector<size_t> &num_views);
