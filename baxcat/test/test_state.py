...
# ---
def test_predictive_logp_unconstrained_invalid_input(blank):
    bcstate = blank
    with pytest.raises(ValueError):
        bcstate.predictive_logp([[2, 2], [1, 1]], [0.0])
    with pytest.raises(ValueError):
        bcstate.predictive_logp([[2, 2]], [0.0, 1.1])


def test_predictive_logp_unconstrained_valid_output(blank):
    bcstate = blank
    res = bcstate.predictive_logp([[2, 2]], [0.0])
...
def test_predictive_logp_unconstrained_j_query_valid_output(blank):
    bcstate = blank
    res = bcstate.predictive_logp([[2, 2], [0, 1]], [0.0, 1.2])
...
# ---
def test_predictive_logp_constrained_invalid_input(blank):
    bcstate = blank
    with pytest.raises(ValueError):
        bcstate.predictive_logp([[2, 2]], [0.0], [[0, 0]], [1.2, 1.1])
    with pytest.raises(ValueError):
        bcstate.predictive_logp([[2, 2]], [0.0], [[0, 0], [1, 2]], [1.])


def test_predictive_logp_constrained_valid_output(blank):
    bcstate = blank
    res = bcstate.predictive_logp([[2, 2]], [0.0], [[0, 0]], [1.2])
...
def test_predictive_logp_constrained_query_valid_output(blank):
    bcstate = blank
    res = bcstate.predictive_logp([[2, 2], [0, 1]], [0.0, 1.2],
                                  [[0, 0]], [1.2])
